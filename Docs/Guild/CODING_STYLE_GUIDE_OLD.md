...
### Field Order
```csharp
[Header("Descriptive Context Header")]  // Optional: describes what this script does
[Header("References")]                   // 1. Components (private, aligned)
C_Health c_Health;
C_Stats  c_Stats;

[Header("MUST wire MANUALLY in Inspector")]  // 2. UI/Prefabs (public)
public TMP_Text goldText;

[Header("Movement Settings")]           // 3. Public Settings (deep indent align)
                public float moveSpeed = 5f;
[Range(0, 100)] public float accuracy  = 75f;

// Runtime state                        // 4. Private variables (last, no header)
bool isDead;
```
...
void Awake()
{
    ...
    // 2. Validate required References (aligned, with this)
...
### Debug Format
```csharp
Debug.LogError($"{name}: Component is missing!", this);    // Required (return after)
Debug.LogWarning($"{name}: Component is missing!", this);  // Optional (no return)
```
