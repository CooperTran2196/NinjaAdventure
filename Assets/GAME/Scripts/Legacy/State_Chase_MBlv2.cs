...
public class State_Chase_MBlv2 : MonoBehaviour
{
    [Header("Tuning")]
    ...
    void Awake()
    {
        ...
        if (!stats) Debug.LogError($"{name}: C_Stats missing in State_Chase_MBlv2");
        if (!anim)  Debug.LogError($"{name}: Animator missing in State_Chase_MBlv2");
        if (controller == null) Debug.LogError($"{name}: I_Controller missing for State_Chase_MBlv2");
    }
